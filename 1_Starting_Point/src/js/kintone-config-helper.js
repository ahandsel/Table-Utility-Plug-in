(function (global, factory) {
  ...
  ***************************************************************************** */
  ...
  function createGetFields(kintoneClient, Promise_) {
    ...
    var fetchFormInfoByFields = kintoneClient.fetchFormInfoByFields,
      fetchFormInfoByLayout = kintoneClient.fetchFormInfoByLayout;

    function removeUnnecessaryProperties(field) {
      var _ = field.size,
        rest = __rest(field, ["size"]);
      return rest;
    }
    ...
    function getFieldInfo(layoutFields) {
      return layoutFields
        .filter(function (layout) {
          return layout.type !== "LABEL" && layout.type !== "HR";
        })
        .map(function (field) {
          return removeUnnecessaryProperties(field);
        });
    }
    ...
    function modifiedLayoutResp(layoutList) {
      return layoutList.reduce(function (acc, layout) {
        switch (layout.type) {
          ...
          case "SUBTABLE":
            var layoutFieldsIncludeSubtableCode = layout.fields.map(function (layoutField) {
              return (__assign({}, layoutField, {
    ...
    function addLabel(layoutFieldList, fieldsResp) {
      ...
      return layoutFieldList.map(function (layoutField) {
        return labeledFields[layoutField.code] ?
          __assign({}, layoutField, {
            label: labeledFields[layoutField.code]
          }) : layoutField;
      });
    }

    function getLookupFieldKeys(fieldsResp) {
      return Object.keys(fieldsResp).filter(function (key) {
        return typeof fieldsResp[key].lookup !== "undefined";
      });
    }
    ...
    function filterLookupField(layoutFieldList, fieldsResp) {
      ...
      return layoutFieldList.filter(function (layoutField) {
        return !lookupFieldKeys.some(function (key) {
          return fieldsResp[key].code === layoutField.code;
        });
      });
    }
      ...
      return Promise.all([fetchFormInfoByFields(), fetchFormInfoByLayout()]).then(function (_a) {
        ...
        return selectFieldTypes ?
          fieldList.filter(function (field) {
            return selectFieldTypes.indexOf(field.type) !== -1;
          }) :
          fieldList;
      });
    }
    ...
    function validateFieldType(fieldType) {
      return ALL_FIELD_TYPES.some(function (type) {
